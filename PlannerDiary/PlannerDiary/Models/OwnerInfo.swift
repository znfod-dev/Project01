...
extension OwnerInfo: NSCopying {
	...
	static func SQLExcute(command: String, condition: String?, dicFields: [String: String]?) -> Results<Object>? {
		...
		if command == "INSERT" {
			let newObject = OwnerInfo(dicFields!)
			
			// isPrimaryKey는 프라이머리키 설정 했는지 유무
			DBManager.sharedInstance.insertSQL(objs: newObject, isPrimaryKey: true)
		}
		else if command == "UPDATE" {
			// 조건식 검색해서 존재할 경우
			let objects = DBManager.sharedInstance.selectSQL(type: OwnerInfo.self, condition: condition ?? "")?.first
			if objects != nil {
				let newObject: OwnerInfo = objects?.copy() as! OwnerInfo
