...
class ModelDBHoliday: Object, NSCopying {
    ...
    static func SQLExcute(command: String, condition: String?, dicFields: [String: String]?) -> Results<Object>? {
        ...
        if command == "INSERT" {
            let newPerson = ModelDBHoliday(dicFields!)
            
            // isPrimaryKey는 프라이머리키 설정 했는지 유무
            DBManager.sharedInstance.insertSQL(objs: newPerson, isPrimaryKey: false)
        }
        else if command == "UPDATE" {
            // 조건식 검색해서 존재할 경우
            let objects = DBManager.sharedInstance.selectSQL(type: ModelDBHoliday.self, condition: condition ?? "")?.first
            if objects != nil {
                let newPerson: ModelDBHoliday = objects?.copy() as! ModelDBHoliday
                
                // 변경된 내용 수정
                newPerson.SQLParsing(dicFields!)
                
                // isPrimaryKey는 프라이머리키 설정 했는지 유무
                DBManager.sharedInstance.updateSQL(objs: newPerson, isPrimaryKey: false)
            }
        }
